...
var Auth = {
  ...
  hash: async function(p) {
    /* crypto.subtle فقط در «بستر امن» وجود دارد: https یا localhost.
       اگر برنامه را با http روی آی‌پی شبکه باز کنید (مثلاً از گوشی با
       http://192.168.1.5:8000) این شیء undefined است و نسخه قبلی با
       پیام گیج‌کننده «Cannot read properties of undefined» از کار
       می‌افتاد و کاربر نمی‌فهمید چرا وارد نمی‌شود. */
    if (!self.crypto || !self.crypto.subtle) {
      throw new Error(
        'برای ورود امن، برنامه باید با https یا از طریق localhost باز شود. ' +
        'آدرس فعلی (' + location.protocol + '//' + location.hostname + ') ' +
        'بستر امن نیست.'
      );
    }
  ...
  login: async function() {
    try {
      ...
      if (user.active === false) {
        errEl.textContent = 'این کاربر غیرفعال شده است. با مدیر سیستم تماس بگیرید.';
        errEl.style.display = 'block';
        return;
      }
