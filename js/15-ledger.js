...
var Led = {
  ...
  show: async function(cid) {
    ...
    invs.forEach(function(inv) {
      ...
      if (inv.contactId === cid) {
        if (inv.type === 'sale') {
          ...
          /* ردیف «دریافت فاکتور» اینجا ساخته نمی‌شود؛ فیلد «پرداخت شده»
             فاکتور یک سند دریافت واقعی می‌سازد که در حلقه pays پایین‌تر
             به دفتر اضافه می‌گردد. قبلاً هر دو ثبت می‌شد و دفتر معین دو
             ردیف تکراری نشان می‌داد. */
        } else {
          txs.push({
            d: inv.date,
          ...
          /* همان توضیح بالا برای فاکتور خرید */
        }
      }
    ...
    pays.forEach(function(pay) {
      /* سند خودکارِ وصول چک در دفتر معین نمی‌آید؛ خودِ چک ردیف خودش را
         دارد و این سند فقط اثر بانکی دارد (وگرنه دو ردیف تکراری). */
